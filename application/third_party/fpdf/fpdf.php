...
function addCadreEurosFrancs($hrg_satuan, $ppn, $hrg_total, $catatan)
{
	$r1  = $this->w - 200;
	$r2  = $r1 + 190;
	...
	$this->Line( $r1+152,  $y1, $r1+152, $y2); // Entre Euros & Francs
	$this->Line( $r1+115,  $y1, $r1+115 , $y2); // Entre Euros & Francs
	$this->Line( $r1+115, $y1+7, $r2, $y1+7); // Sous Euros & Francs
	$this->Line( $r1+115, $y1+14, $r2, $y1+14); // Sous Euros & Francs
	$this->Line( $r1+14, $y1+44, $r2-149, $y1+44); // Sous Euros & Francs
	...
	#$this->SetFont( "Arial", "B", 6);
	$this->SetXY( $r1, $y1+5 );
	$this->Cell(250,-2, "Sub Total", 0, 0, "C");
	$this->SetXY( $r1, $y1+4 );
	$this->Cell(0, 0, "Rp. ".str_replace(",",".", number_format($hrg_satuan)), 0, 0, "R");
	$this->SetXY( $r1, $y1+10 );
	$this->Cell(249,1, "PPN 10%", 0, 0, "C");
	$this->SetXY( $r1, $y1+11 );
	$this->Cell(0, 0, "Rp. ".str_replace(",",".", number_format($ppn)), 0, 0, "R");
	$this->SetXY( $r1, $y1+15 );
	$this->Cell(257.5, 4, "GRAND TOTAL", 0, 0, "C");
	$this->SetXY( $r1, $y1+18 );
	$this->Cell(0, 0, "Rp. ".str_replace(",",".", number_format($hrg_total)), 0, 0, "R");
	...
	$this->SetXY( $r1, $y1+5 );
	$this->Cell(55,-5, "Mengetahui", 0, 0, "C");
	$this->SetXY( $r1, $y1+8 );
	$this->Cell(60,-3, "PT. PELANGI LINTAS SEJAHTERA ", 0, 0, "C");
	$this->SetXY( $r1, $y1+12 );
	$this->Cell(55,20, "HARI SANTOSO.S.E", 0, 0, "C");
	$this->Cell(-55,26, "DIREKTUR", 0, 0, "C");
	$this->SetXY( $r1, $y1+12 );
	
	$this->SetXY( $r1, $y1+15 );
	$this->SetFont( "Arial", "", 7);
	$this->Cell(295,23, "CONTACT PERSON : Yanuar : 0852131259241", 0, 0, "C");
...
function addReglement( $mode )
{
	...
	$this->SetFont( "Arial", "B", 8);
	$this->Cell(38,110, "HARGA PENAWARAN PENGADAAN BARANG", 0, 0, "R");
...
// VAT number
function addNumTVA($tva, $nmMitra, $noSpph, $alamat)
{
	...
	$y2  = $y1+0;
	$mid = $y1 + (($y2-$y1));
	$this->RoundedRect($r1, $y1, ($r2 - $r1), 25, 'D');
	$this->Line( $r1, $mid, $r2, $mid);
	$this->SetXY( $r1 + 0 , $y1+1 );
	$this->Cell(10, 4, "TO        : ".$nmMitra, 0, 0, "L");
	$this->SetXY( $r1 + 0 , $y1+5 );
	$this->MultiCell(90, 5, "Alamat : ".$alamat, 10);

	
	// $this->SetXY( $r1 + 6.5 , $y1+6 );
	// $this->Cell(10, 4, "Dept. Pengadaan Barang - Electrical", 0, 0, "L");
	
	$this->SetXY( $r1 + 135 , $y1+1 );
	$this->Cell(10, 4, "Penawaran", 0, 0, "L");
	$this->SetXY( $r1 + 135 , $y1+6 );
	$this->Cell(10, 4, "Tanggal :   ".date('d')."    /   ".$month[date($bln)]."    /   ".date('Y')."  ", 0, 0, "L");
	$this->SetXY( $r1 + 135 , $y1+11 );
	$this->Cell(10, 4, "I.H										: ".$noSpph, 0, 0, "L");
